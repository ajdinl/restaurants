...
async function migrate() {
    try {
        ...
        for (const restaurant of restaurants) {
            ...
            console.log(
                `\nRestaurant: ${restaurant.name} - ${orders.length} orders, ${reservations.length} reservations`
            );

            for (const order of orders) {
                if (order.table_number && !order.table_id) {
                    const table = restaurantTables.find((t) => t.number === order.table_number);
                    if (table) {
                        ...
                        console.log(`  ✗ No table found for order #${order.number} (table ${order.table_number})`);
                    }
                }
            }
            ...
            for (const reservation of reservations) {
                if (reservation.table_number && !reservation.table_id) {
                    const table = restaurantTables.find((t) => t.number === reservation.table_number);
                    if (table) {
                        ...
                        console.log(
                            `  ✗ No table found for reservation #${reservation.number} (table ${reservation.table_number})`
                        );
                    }
                }
            }
