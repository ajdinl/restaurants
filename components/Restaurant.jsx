...
export default function RestaurantComponent({ id }) {
    ...
    return (
        <>
            {loading && <div className='text-center text-xl font-bold mt-96 text-gray-500'>Loading...</div>}
            {completed && <div className='text-center text-xl font-bold mt-96 text-gray-500'>Your Delicious Order Has Been Placed</div>}
            {!loading && !completed && restaurant && (
                <div className='flex flex-col'>
                                            ...
                                            <>
                                                <div className='flex flex-col items-center mt-1'>
                                                    <div className='flex flex-row space-x-2 -mt-1'>
                                                        <label className='text-gray-600 hidden sm:inline'>Quantity:</label>
                                                        <p>{order[key]}</p>
                                                    </div>
                                                </div>
                                                <div className='flex flex-col sm:flex-row justify-between text-md mb-2 w-full'>
                                                    ...
                                                    <p className='sm:ml-10'>${order[key] * menu[0].items.find((item) => item.name === key).price}</p>
                                                </div>
                                            </>
                                        )}
                                    </div>
                    ...
                    <div className='flex flex-col items-center'>
                        <Button className='bg-green-500 hover:bg-green-600 text-white mt-6' onClick={() => handleOrder(order)}>
                            Order
                        </Button>
                    </div>
                </div>
