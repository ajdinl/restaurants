                ...
                <div className='relative w-1/4 min-w-96 my-6 mx-auto max-w-3xl'>
                    <div className='border-0 rounded shadow-lg relative flex flex-col w-full bg-white dark:bg-gray-800 dark:shadow-md dark:shadow-gray-500/50 outline-none focus:outline-none'>
                        <div className='flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t'>
                            <h3 className='text-3xl font-semibold dark:text-white'>Edit Modal</h3>
                            <button className='p-1 ml-auto bg-transparent border-0 text-red-500 opacity-80 float-right text-xl leading-none font-semibold outline-none focus:outline-none' onClick={() => setShowEditModal(false)}>
                                X
                            </button>
                        </div>
                        <div className='relative p-6 flex-auto'>
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Edit the {removeLastChar(selected.category)}.</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    {selected.category === 'tables' && <SelectInput label='Maximum Capacity' value={capacity} onChange={(e) => setCapacity(e.target.value)} />}
                                    ...
                                    {selected.category === 'reservations' && (
                                        <form className='w-full'>
                                            <div className='dropdown'>
                                                <Button className={status === 'Available' ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-red-500 hover:bg-red-600 text-white'} onClick={handleOpen}>
                                                    {status}
                                                </Button>
                                                {openDropdown && (
                                                    <ul className='menu'>
                                                        {status === 'Available' && (
                                                            <li className='menu-item' onClick={() => setStatusValue('Reserved')}>
                                                                <Button className='mt-4 bg-red-500 hover:bg-red-600 text-white'>Reserved</Button>
                                                            </li>
                                                        )}
                                                        {status === 'Reserved' && (
                                                            <li className='menu-item' onClick={() => setStatusValue('Available')}>
                                                                <Button className='mt-4 bg-green-500 hover:bg-green-600 text-white'>Available</Button>
                                                            </li>
                                                        )}
                                                    </ul>
                                                )}
                                            </div>
                                            <SelectInput label='Number of Guests' value={capacity} onChange={(e) => setCapacity(e.target.value)} />
                                        </form>
                                    )}
                                </CardContent>
                            </Card>
                        ...
                        <div className='flex items-center justify-end p-6 border-t border-solid border-blueGray-200 rounded-b'>
                            <button className='text-red-500 background-transparent font-bold uppercase px-6 py-2 text-sm outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150' type='button' onClick={() => setShowEditModal(false)}>
                                Close
                            </button>
                            <button className='bg-emerald-500 text-white active:bg-emerald-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150' type='button' onClick={() => handleEditSave()}>
                                Save Changes
                            </button>
                        </div>
                    </div>
